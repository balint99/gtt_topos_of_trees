...
\begin{enum}
    ...
    \[ \bar{A}_n(x) = \begin{cases}
                          ...
                          \nul & \text{ if $x \notin A$}
                      \end{cases} \]
    More abstractly, let $\2 = \{\btrue, \bfalse\}$ denote the set of booleans. There is a morphism $c : \2 \to \SOC$ whose $n$-th component sends $\btrue$ to $\fin{n}$ and $\bfalse$ to $\nul$. Assuming a classical metatheory, subsets $A$ of $X$ correspond to \emph{characteristic functions} $\chi_A : X \to \2$. Then $\bar{A}$ is simply described as the composite $X \xrightarrow{\chi_A} \2 \xrightarrow{c} \SOC$.
