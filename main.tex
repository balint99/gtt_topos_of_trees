...
\begin{items}
    ...
    \item Finally, we have formalised the topos of trees in the Coq theorem prover~\cite{coq}, including a shallow embedding of its internal logic. We provide details and technical insights in the report.
\end{items}

In \cref{sec:tot}, we review the main semantic definitions in the topos of trees. Then we study its internal logic in \cref{sec:internal-logic}. In \cref{sec:coq}, we elaborate on some design choices and technical challenges of our Coq formalisation. Related work is discussed in \cref{sec:related}. Finally, \cref{sec:conclusion} concludes with a summary and future directions.
...
For formalisation purposes in a constructive type theory such as Coq or Agda, it is crucial that all functions be computable. This can be achieved in the second model, where the interpretation of the quantifiers is given by
\begin{align*}
...
In the following, we discuss some interesting aspects of the formalisation, including some of the design choices and technical challenges. \Cref{sec:finite-types} motivates our definition of finite types. \Cref{sec:proof-irrelevance} shows how we work with proof-irrelevant propositions. \Cref{sec:coq-obj-mor,sec:coq-exp} explain some general techniques we used in our formalisation via the examples of objects, morphisms, and exponentials. \Cref{sec:coq-logic} expands on the concept of a shallow embedding and how we applied it to formalise the internal logic. Finally, \cref{sec:coq-axioms} touches on the axioms we relied on and how their use could be eliminated.
...
As stated in \cref{def:subobject-classifier}, the $n$-th component of the subobject classifier is the collection of downward closed subsets of $\fin{n}$. In type theory, subsets $A$ of a set $X$ are usually formalised as predicates \coqi{A : X → Prop}, where $x \in A$ iff \coqi{A x} holds. Hence, a reasonable formalisation of $\SOC_n$ would be the following record type.
\begin{coqc}
...
Lemma conj_intro {Γ} {R P Q : Γ ⟶ Ω} :
  ...
  R ⊢ conj P Q.
\end{coqc}
Perhaps a bit confusingly, we use the \coqi{⊢} symbol for provability in the formalisation. This is because we would like to treat the shallow embedding of the internal language as a combinator calculus, so we prefer to use syntactic notations.
