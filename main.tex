...
The idea of step-indexing can be formalised in a logical, or type-theoretical, setting, by introducing a modality, called later, which allows us to talk about the next computation step.

In this work, we give an introduction to the logic of step-indexing, with a particular emphasis on its semantics in the topos of trees. We investigate sound rules in the internal logic of the topos of trees for commuting the later modality with quantifiers. Furthermore, we provide a Rocq mechanisation of the topos of trees, along with a shallow embedding of its internal logic.
\end{abstract}
...
\subsection{Formalisation} \label{sec:intro-formalisation}

It seems that not much of the structure of the topos of trees has been formalised in the literature. To fill this hole, we formalise most constructions, definitions, and propositions in the present report. The original goal of the formalisation was to experiment with a model of Iris constructed using the internal logic of the topos of trees, as alluded to in \cref{sec:intro-guarded-rec}. This is however left to future work.

An important insight gained from the formalisation concerns the representation of finite types.
Our experiments have shown that the traditional inductive representation of the natural number indexed family \rocqi{fin n} of finite types seemed to entail messy type coercions. Representing elements of \rocqi{fin n} as natural numbers equipped with a proof that they are less than $n$ proved to be more fruitful. The details are given in \cref{sec:finite-types}.
...
\begin{items}
    ...
    \item Finally, we have formalised the topos of trees in the Rocq theorem prover~\cite{rocq}, including a shallow embedding of its internal logic. We discuss details and technical insights in the report.
\end{items}

In \cref{sec:tot}, we review the main semantic definitions in the topos of trees. Then we study its internal logic in \cref{sec:internal-logic}. In \cref{sec:rocq}, we elaborate on some design choices and technical challenges of our Rocq formalisation. Related work is discussed in \cref{sec:related}. Finally, \cref{sec:conclusion} concludes with a summary and future directions.
...
The \emph{topos of trees} (denoted by $\TOT$) is the category of presheaves on $\omega$, the set of
...
\begin{enum}
    ...
    \item One may wonder if it is possible to generalise the previous example to all
...
\[ \mor{fix}_X = \ev \circ \mprod{\fix{(\curry{g})} \circ \mterm}{\id}, \]
where $g$ is the middle horizontal composite in the diagram
...
\end{tikzcd}\]
That is,
\[ g = \ev \circ \mprod{\mprojr}{\ev \circ (J \times \mnxt)} \]
\end{defn}
...
\[ \name{fix}(f) =
    \app{(\rec{h}{\Later{((\Later{X} \to X) \to X)}}{
        \lam{f}{\Later{X} \to X}{\app{f}{(h \ap \nxt{f})}}})} f. \]
...
\end{align*}
It follows from the definitions prove that if $A$ and $B$ are downward closed, so are $A \wedge B$ and $A \vee B$; $A \impl B$ is downward closed by definition. Semantic entailment is given by the subset relation.
...
For formalisation purposes in a constructive type theory such as Rocq or Agda, it is crucial that all functions be computable. This can be achieved in the second model, where the interpretation of the quantifiers is given by
\begin{align*}
...
The \emph{internal logic} $\lang$ of $\TOT$ is a typed higher-order logic, whose types are generated by the grammar
...
\Cref{fig:later-rules} lists some important inference rules for the $\later$ modality. Of particular interest is the rule \ruleref{rule:löb}, called \emph{Löb induction}. This principle internalises the technique of induction on the step-index, and it is the main tool for proving properties of guarded recursively defined programs. Note the formal similarity between \ruleref{rule:löb} and the fixed point operator of \cref{prop:fixpoint}. Another important rule is \ruleref{rule:later-eq}, which establishes a strong connection between the $\later$ modality and the $\Later$ type former.
\begin{figure}
\begin{mathpar}
\inferhref{$\later$-intro}{rule:later-intro}{}{\entails{\Gamma}{P}{\later{P}}}
\and
\inferhref{$\later$-mono}{rule:later-mono}
    {\entails{\Gamma}{P}{Q}}
    {\entails{\Gamma}{\later{P}}{\later{Q}}}
\and
\inferhref{löb}{rule:löb}
    {\entails{\Gamma}{\later{P}}{P}}
    {\valid{\Gamma}{P}}
\and
\inferhref{$\wedge$-$\later$}{rule:and-later}{}
    {\entails{\Gamma}{\later{P} \wedge \later{Q}}{\later{(P \wedge Q)}}}
\\
\inferhref{$\later$-$\vee$}{rule:later-or}{}
    {\entails{\Gamma}{\later{(P \vee Q)}}{\later{P} \vee \later{Q}}}
\and
\inferhref{$\impl$-$\later$}{rule:impl-later}{}
    {\entails{\Gamma}{\later{P} \impl \later{Q}}{\later{(P \impl Q)}}}
\and
\inferhref{$\later$-eq}{rule:later-eq}{}
...
The proof of \ruleref{rule:strong-löb} is less trivial, but it closely resembles the construction of the internal fixed point operator of \cref{def:fix} from the external one. Finally, we can use \ruleref{rule:strong-löb} to prove uniqueness of fixpoints (\ruleref{rule:fix-eta}).

\begin{figure}
\begin{mathpar}
\inferhref{strong-löb}{rule:strong-löb}{}{\entails{\Gamma}{\later{P} \impl P}{P}}
\and
\inferhref{$\later$-$\wedge$}{rule:later-and}{}
    {\entails{\Gamma}{\later{(P \wedge Q)}}{\later{P} \wedge \later{Q}}}
\and
\inferhref{$\vee$-$\later$}{rule:or-later}{}
    {\entails{\Gamma}{\later{P} \vee \later{Q}}{\later{(P \vee Q)}}}
\\
\inferhref{$\later$-$\impl$}{rule:later-impl}{}
    {\entails{\Gamma}{\later{(P \impl Q)}}{\later{P} \impl \later{Q}}}
\and
\inferhref{$\exists$-$\later$}{rule:exists-later}{}
    {\entails{\Gamma}{\exi{x}{A}{\later{P}}}{\later{(\exi{x}{A}{P})}}}
\and
\inferhref{$\later$-$\forall$}{rule:later-forall}{}
    {\entails{\Gamma}{\later{(\all{x}{A}{P})}}{\all{x}{A}{\later{P}}}}
\and
\inferhref{fix-$\eta$}{rule:fix-eta}
    {\typed{\Gamma, x : \Later{A}}{t}{A} \and \typed{\Gamma}{u}{A}}
    {\entails{\Gamma}{u =_A \subst{t}{x}{\nxt{u}}}{u =_A (\rec{x}{\Later{A}}{t})}}
\end{mathpar}
\caption{Derived rules in $\lang$}
...
\app{\name{add}}{(m + n)} =_{\Str \to \Str}
    \app{\app{\name{comp}}{(\app{\name{add}}{m})}}{(\app{\name{add}}{n})}
\end{equation*}
for all $m, n : \N$, where $\name{comp} = \lam{f}{B \to C}{\lam{g}{A \to B}{\lam{x}{A}{\app{f}{(\app{g}{x})}}}}$ is function composition. Since $\name{add}$ was defined as a fixed point, we will eventually need to handle a recursive call. Hence, we use Löb induction (\ruleref{rule:löb}) to get an induction hypothesis
\begin{equation*}
\later{(\app{\name{add}}{(m + n)} =_{\Str \to \Str}
    \app{\app{\name{comp}}{(\app{\name{add}}{m})}}{(\app{\name{add}}{n})})}.
\end{equation*}
By function extensionality, it suffices to prove the equality once both sides are applied to an argument stream $s : \Str$. Unfolding the definition of $\name{add}$, and using the appropriate $\beta$-rules, including one application of \ruleref{rule:fix-beta}, the left hand side becomes
\begin{equation*}
...
\end{equation*}
The heads of the two streams are equal (by associativity and commutativity of addition), so it suffices to prove the equality of the tails. By \ruleref{rule:later-eq}, our induction hypothesis is equivalent to
\begin{equation*}
\nxt{(\app{\name{add}}{(m + n)})} =_{\Later{(\Str \to \Str)}}
    \nxt{(\app{\app{\name{comp}}{(\app{\name{add}}{m})}}{(\app{\name{add}}{n})})}.
\end{equation*}
Now we conclude the proof using \ruleref{rule:ap-next} and \ruleref{rule:ap-comp}:
\begin{align*}
\nxt{(\app{\name{add}}{(m + n)})} \ap \app{\tl}{s}
    &= \nxt{(\app{\app{\name{comp}}{(\app{\name{add}}{m})}}{(\app{\name{add}}{n})})} \ap \app{\tl}{s} \\
    &= \nxt{\name{comp}} \ap \nxt{(\app{\name{add}}{m})} \ap \nxt{(\app{\name{add}}{n})} \ap \app{\tl}{s} \\
...
\begin{mathpar}
\inferhref{$\later$-$\exists$-comm}{rule:later-exists-comm}{}{\equiv{\Gamma}{\later{(\exi{x}{A}{P})}}{\exi{x}{A}{\later{P}}}}
\and
\inferhref{$\later$-$\forall$-comm}{rule:later-forall-comm}{}{\equiv{\Gamma}{\later{(\all{x}{A}{P})}}{\all{x}{A}{\later{P}}}}
\end{mathpar}

As the rules \ruleref{rule:exists-later} and \ruleref{rule:later-forall} show, one direction of both equivalences is derivable in the system $\lang$. In both cases, however, the other direction is not semantically valid. To explain why, we first characterise logical validity in $\TOT$ in a more intuitive way, known in the literature as \emph{Kripke-Joyal semantics}~\cite{lambek:scott,maclane:moerdijk}. This characterisation is formulated in terms of a \emph{forcing relation} $\force{}{}$: for $\typed{\Gamma}{P}{\Prop}$, $n \in \N$, and $\gamma \in \sem{\Gamma}_n$, we define $\force{n}{P(\gamma)}$ iff $n \in \sem{P}_n(\gamma)$. Intuitively, $\force{n}{P(\gamma)}$ means that $P$ holds at step-index $n$, where the parameters of $P$ have been substituted with the semantic values $\gamma$. If $\Gamma$ is the empty context, we simply write $\force{n}{P}$.
...
Similarly, we calculate the meaning of the two sides of \ruleref{rule:later-forall-comm}:
\begin{align*}
...
For a total object $X$, considered as a type in $\lang$, we have $\force{n+1}{\later{(\exi{x}{X}{P})}}$ if and only if $\force{n+1}{\exi{x}{X}{\later{P}}}$, and similarly for $\forall$.  Since $\force{0}{\later{(\all{x}{X}{P})}}$ and $\force{0}{\all{x}{X}{\later{P}}}$ always hold, we get that the rule \ruleref{rule:later-forall-comm} is valid for total objects $X$. For \ruleref{rule:later-exists-comm} to hold at level 0, we need the additional assumption that $X_0$ is inhabited. For total objects $X$, this condition is equivalent to the existence of a morphism $\1 \to X$, i.e. being inhabited by a global element.
...
Thus, the rule \ruleref{rule:later-exists-comm} holds for total and inhabited $X$. It turns out that we can characterise this property in the internal logic.
...
\begin{mathpar}
\inferhref{$\later$-$\exists$-comm-ti}{rule:later-exists-comm-ti}
  {\valid{}{\name{TI}(A)}}
  {\equiv{\Gamma}{\later{(\exi{x}{A}{P})}}{\exi{x}{A}{\later{P}}}}
\and
\inferhref{$\later$-$\forall$-comm-ti}{rule:later-forall-comm-ti}
  {\valid{}{\name{TI}(A)}}
  {\equiv{\Gamma}{\later{(\all{x}{A}{P})}}{\all{x}{A}{\later{P}}}}
\end{mathpar}
It would be nice if we could derive these rules in the system $\lang$. Unfortunately, this does not seem to be possible. Hence, there arises a natural question: is there a more general rule, working uniformly for all types $A$, from which \ruleref{rule:later-exists-comm-ti} and \ruleref{rule:later-forall-comm-ti} could be derived? We make two observations that will lead us to answer this question.
...
Then, by \ruleref{rule:ap-next}, we have
...
\begin{mathpar}
\inferhref{lift-$\exists$-comm}{rule:lift-exists-comm}
  {\typed{\Gamma}{Q}{\Later{(A \to \Prop)}}}
  {\equiv{\Gamma}{\lift{(\nxt{\mex} \ap Q)}}
                 {\exi{y}{\Later{A}}{\lift{(Q \ap y)}}}}
\and
\inferhref{lift-$\forall$-comm}{rule:lift-forall-comm}
  {\typed{\Gamma}{Q}{\Later{(A \to \Prop)}}}
  {\equiv{\Gamma}{\lift{(\nxt{\mall} \ap Q)}}
...
The rules \ruleref{rule:later-exists-comm-ti} and \ruleref{rule:later-forall-comm-ti} can indeed be derived from the new rules. We show how to do this for the existential quantifier; the universal quantifier is analogous. Given $P$ and writing $\tilde{P} = \lam{x}{A}{P}$, we have
\[ \later{(\exi{x}{A}{P})} = \lift{(\nxt{(\exi{x}{A}{\app{\tilde{P}}{x}})})} =
  \lift{(\nxt{\mex} \ap \nxt{\tilde{P}})} \]
by \cref{eq:next-exists}. By \ruleref{rule:lift-exists-comm}, this holds iff $\exi{y}{\Later{A}}{\lift{(\nxt{\tilde{P}} \ap y)}}$. Since $A$ is total and inhabited, this is equivalent to $\exi{x}{A}{\lift{(\nxt{\tilde{P}} \ap \nxt{x})}}$. But
\[ \lift{(\nxt{\tilde{P}} \ap \nxt{x})} = \lift{(\nxt{(\app{\tilde{P}}{x})})} =
   \later{P} \]
using \ruleref{rule:ap-next}, so we are done.

The generality of the new rules (\ruleref{rule:lift-exists-comm} and \ruleref{rule:lift-forall-comm}) indicates that $\mlift$ should be taken as the primitive logical connective, with $\later{P} = \lift{(\nxt{P})}$ being a defined connective. This claim is also supported by the necessity of $\mlift$ for defining guarded recursive predicates (\cref{ex:predicate-programs}). However, it is not yet clear what the analogues of e.g. monotonicity or Löb induction would be, or if such analogues exist. This question is further discussed in \cref{sec:conclusion}.

\section{Rocq formalisation} \label{sec:rocq}

We have formalised the topos of trees, along with the associated structure introduced so far, in the Rocq theorem prover~\cite{rocq}. More specifically, the formalisation encompasses the basic categorical structure (\cref{sec:basic-struct}), the definitions necessary for guarded recursion (\cref{sec:guarded-rec}), and the logical components of \cref{sec:logic-definitions}. Moreover, we have verified semantically some of the inference rules of the internal logic, in particular the novel rules of \cref{sec:later-quantifiers}.

The formalisation does not contain the examples in this report. It is important to note that we do not define a syntax for the internal logic, as we did in \cref{sec:internal-logic-def}. Instead, we work with the semantics directly, and use combinators to construct terms. The inference rules of the logic are then stated and proved as lemmas. In other words, we use a \emph{shallow embedding}~\cite{wildmoser:2004:tphols} of the internal logic. This contrasts with a \emph{deep embedding}~\cite{wildmoser:2004:tphols} of the syntax as an inductive definition, together with an interpretation function to show soundness, as was done in \cref{def:logic-semantics}.

In the following, we discuss some interesting aspects of the formalisation, including some of the design choices and technical challenges. \Cref{sec:finite-types} motivates our definition of finite types. \Cref{sec:proof-irrelevance} shows how we work with proof-irrelevant propositions. \Cref{sec:rocq-obj-mor,sec:rocq-exp} explain some general techniques we used in our formalisation via the examples of objects, morphisms, and exponentials. \Cref{sec:rocq-logic} expands on the concept of a shallow embedding and how we applied it to formalise the internal logic. Finally, \cref{sec:rocq-axioms} touches on the axioms we relied on and how their use could be eliminated.

\subsection{Finite types} \label{sec:finite-types}

The definition of the subobject classifier (\cref{def:subobject-classifier}) refers to the set $\fin{n}$ of natural numbers from 0 to $n$. Therefore, we need to represent such finite sets. For this purpose, the Rocq standard library defines an inductive family of types as follows.
\begin{rocqc}
Inductive fin : nat → Type :=
  | FZ {n} : fin (S n)
  | FS {n} : fin n → fin (S n).
\end{rocqc}
Then, \rocqi{fin n} corresponds to the finite set $\fino{n} = \{0, \ldots, n-1\}$: the indices in the types of the constructors ensure that \rocqi{fin n} has exactly $n$ canonical inhabitants.

Given \rocqi{k : fin n}, we often need to treat \rocqi{k} as a natural number, or as being an element of a bigger finite type, e.g. \rocqi{fin (S n)}. In set theory, we can do this since $\fino{n} \subs \fino{n+1} \subs \N$; in Rocq, however, every term has at most one type. Thus, we need conversion functions
\begin{rocqc}
fin_to_nat : ∀ {n}, fin n → nat
FW : ∀ {n}, fin n → fin (S n) 
\end{rocqc}
defined using the eliminator of the type family \rocqi{fin}. But then it is not immediately clear that \rocqi{FW k} and \rocqi{k} represent the same natural number. In particular, equations such as
\begin{equation} \label{eq:fw-lemma}
\mbox{\rocqi{fin_to_nat (FW k) = fin_to_nat k}}
\end{equation}
do not hold by definition and need to be proven by induction on \rocqi{k}. Even worse, such lemmas are necessary to type certain simple expressions, an example of which is given in \cref{sec:rocq-exp}. This is extremely inconvenient and can become quite messy.

Our solution is to define \rocqi{fin n} as the sigma type \rocqi{{m : nat | m < n}}. This closely resembles the set-theoretic definition: \rocqi{fin n} is the \emph{subtype} of \rocqi{nat} consisting of the natural numbers less than \rocqi{n}. The main advantage of this representation is that \rocqi{FW} only changes the second component, and \rocqi{fin_to_nat} is simply the first projection. Thus, \cref{eq:fw-lemma} holds by definition.

To make working with finite types more convenient, we employ Rocq's \emph{definitionally proof-irrelevant} sort \rocqi{SProp} of propositions~\cite{gilbert:2019:popl}. Definitional proof-irrelevance means that if \rocqi{A : SProp}, then any two inhabitants of \rocqi{A} are definitionally equal. In the definition of \rocqi{fin}, we replace the less-than relation \rocqi{<} by its proof-irrelevant counterpart (see \cref{sec:proof-irrelevance}). The effect of this change is that two terms of type \rocqi{fin n} are now convertible iff their first projections are convertible. Thus, once we define helper functions \rocqi{FZ} for zero and \rocqi{FS} for successor, we can treat elements of finite types as if they were natural numbers, with some additional type information.

We can further enhance the situation by declaring the function \rocqi{fin_to_nat} as a Rocq coercion:
\begin{rocqc}
Coercion fin_to_nat : fin >-> nat.
\end{rocqc}
This lets us leave applications of \rocqi{fin_to_nat} implicit.

\subsection{Proof-irrelevant order relations} \label{sec:proof-irrelevance}

To avoid defining custom order relations, we implement the proof-irrelevant versions of \rocqi{≤} and \rocqi{<}, denoted by \rocqi{⪯} and \rocqi{≺} respectively, using \emph{propositional truncation}~\cite[Section 3.7]{hottbook}, called \rocqi{Squash} in Rocq. That is, \rocqi{x ⪯ y} stands for \rocqi{Squash (x ≤ y)}, and \rocqi{x ≺ y} means \rocqi{S x ⪯ y} (which is the same as \rocqi{Squash (x < y)}). Thus, we can easily prove lemmas such as
\begin{rocqc}
Sle_n : ∀ n, n ⪯ n
Sle_S : ∀ {n m}, n ⪯ m → n ⪯ S m
Sle_S_n : ∀ {n m}, S n ⪯ S m → n ⪯ m
\end{rocqc}
by eliminating and introducing \rocqi{Squash} and using the corresponding lemmas for \rocqi{≤}. Unfortunately, this method still requires some boilerplate, as we need to create a new lemma for every lemma we wish to use. It might be possible to derive such lemmas automatically by some form of tactic metaprogramming; we have not tried to do so.

In the next section, we will make use of the following large elimination principle for \rocqi{⪯}:
\begin{rocqc}
Sle_rect : ∀ n m (P : ∀ m, n ⪯ m → Type),
  P n (Sle_n n) → (∀ m (H : n ⪯ m), P m H → P (S m) (Sle_S H)) →
  ∀ H : n ⪯ m, P m H
\end{rocqc}
Due to restrictions on how types in \rocqi{SProp} can be eliminated, we have to define \rocqi{Sle_rect} manually by recursion on \rocqi{m} and \rocqi{n}.
\begin{rem} \label{rem:sprop-large-elim}
The necessity of defining large elimination for the order relation is not the consequence of using \rocqi{SProp} instead of \rocqi{Prop}. Since \rocqi{Prop} cannot be eliminated into \rocqi{Type} either, we would need to act similarly if we used \rocqi{≤}.
\end{rem}

\subsection{Objects and morphisms} \label{sec:rocq-obj-mor}
...
In Rocq, we can conveniently package up such definitions as record declarations. The type of objects is defined as follows.
\begin{rocqc}
Record Object : Type := 
  { obj :> nat → Type
...
\end{rocqc}
The notation \rocqi{obj :> nat → Type} automatically declares the record projection \rocqi{obj} as a coercion. This allows us to write \rocqi{X n} instead of \rocqi{obj X n} for the \rocqi{n}-th component of an object \rocqi{X}. For readability, we keep the first argument of \rocqi{restr} implicit.

We define the more general restriction operations $r^X_{n,m}$ (see \cref{def:tot}) using the elimination principle \rocqi{Sle_rect} mentioned above.
\begin{rocqc}
Definition restr' {X : Object} {m n} : n ⪯ m → X m → X n :=
  Sle_rect n m (λ m _, X m → X n) id (λ m _ f, f ∘ restr m).
\end{rocqc}
As a special case of \rocqi{restr'}, we also have
\begin{rocqc}
Definition restrTo {X : Object} {n} (i : [0..n]) (x : X n) : X i :=
  restr' (Sle_S_n (Spr2 i)) x.
\end{rocqc}
Here, \rocqi{[0..n]} is notation for \rocqi{fin (S n)}, and \rocqi{Spr2 i} is proof that \rocqi{i ≺ S n} (recall that \rocqi{fin n} is defined as a sigma type). This corresponds to the notation $\restr{x}{i}$ introduced in \cref{def:tot}. This use case explains why we need to use \rocqi{⪯} instead of the ordinary \rocqi{≤} for \rocqi{restr'}.
...
/\textcolor{rocqkw}{Infix}/ "⟶" := /\textcolor{black}{Morphism}/.
\end{rocqc}
...
An important difference between objects and morphisms is that morphisms also contain a proof component. That is, whenever we want to construct a morphism \rocqi{X ⟶ Y} (which is a family of maps \rocqi{X n → Y n} for all \rocqi{n}), we have the obligation to show that it satisfies the naturality condition \rocqi{morph_natural}. Typically, we wish to carry out such verifications using Rocq's tactic language instead of constructing proof terms by hand.

A convenient way to achieve this is provided by Rocq's \rocqi{/\textcolor{rocqkw}{Program}/} facilities, originally proposed by Sozeau~\cite{sozeau:2008:phd}. The \rocqi{/\textcolor{rocqkw}{Program}/ Definition} command allows us to leave holes in a definition, which can then be filled in using Rocq's proof mode. As an example, consider the following definition.
\begin{rocqc}
/\textcolor{rocqkw}{Program}/ Definition mcomp {X Y Z} (f : Y ⟶ Z) (g : X ⟶ Y) : X ⟶ Z :=
  ⟦λ n, f n ∘ g n⟧.
/\textcolor{rocqkw}{Next Obligation}/. ... Qed.
\end{rocqc}
Here, \rocqi{⟦h⟧} is notation for \rocqi{{| morph := h; morph_natural := _ |}}, which is Rocq syntax for constructing a record by listing its fields. The underscore is used to defer the proof of naturality, which is then provided using the \rocqi{/\textcolor{rocqkw}{Next Obligation}/} command. If there are multiple holes, they can be solved one-by-one with consecutive applications of \rocqi{/\textcolor{rocqkw}{Next Obligation}/}. We make extensive use of this pattern throughout the formalisation.

\subsection{Exponentials} \label{sec:rocq-exp}

Given \rocqi{X Y : Object}, we may construct the exponential \rocqi{Exp X Y}, denoted by \rocqi{X ⇒ Y}, based on \cref{def:tot-struct}. There, we have seen that the $n$-th component of the exponential $\expt{X}{Y}$ is the collection of $(n+1)$-tuples $(f_i \colon X_i \to Y_i)_{i \in \fin{n}}$ commuting with the restriction maps. Therefore, similarly to \rocqi{/\textcolor{black}{Morphism}/}, we define the \rocqi{n}-th component of \rocqi{Exp X Y} as a record.
\begin{rocqc}
...
\end{rocqc}
Here, \rocqi{[0..n|]} and \rocqi{[0..n]} are notations for \rocqi{fin n} and \rocqi{fin (S n)}, respectively.

It is worth noting a subtle but important point regarding the definition of \rocqi{Exp_obj}. In the expression \rocqi{Exp_morph (FW i) (restr i x)} in the type of \rocqi{Exp_morph_natural}, the second argument of \rocqi{Exp_morph} is supposed to have type \rocqi{X (fin_to_nat (FW i))}. However, \rocqi{restr i x} has type \rocqi{X (fin_to_nat i)}. Thus, the well-typedness of this expression crucially depends on \cref{eq:fw-lemma} in \cref{sec:finite-types} holding definitionally. Had we chosen the alternative representation of \rocqi{fin} as an inductive family, it would be necessary to transport along this equation explicitly. This would make reasoning with \rocqi{Exp_morph_natural} (and in general, \rocqi{Exp_morph}) much more difficult.

Similarly, the type of \rocqi{x} in \rocqi{Exp_morph (FS i) x} is \rocqi{X (S (fin_to_nat i))}, but its expected type is \rocqi{X (fin_to_nat (FS i))}. Thus, the expression is well-typed only because the equation
\begin{equation} \label{eq:fs-lemma}
\mbox{\rocqi{fin_to_nat (FS i) = S (fin_to_nat i)}}
\end{equation}
holds definitionally. However, this would also be the case with the inductive definition of \rocqi{fin}, where \cref{eq:fs-lemma} would be one of the defining equations for \rocqi{fin_to_nat}.

\subsection{Logic} \label{sec:rocq-logic}

As stated in \cref{def:subobject-classifier}, the $n$-th component of the subobject classifier is the collection of downward closed subsets of $\fin{n}$. In type theory, subsets $A$ of a set $X$ are usually formalised as predicates \rocqi{A : X → Prop}, where $x \in A$ iff \rocqi{A x} holds. Hence, a reasonable formalisation of $\SOC_n$ would be the following record type.
\begin{rocqc}
...
\end{rocqc}
However, we can exploit the order structure on the natural numbers to simplify the closedness requirement. We replace the field \rocqi{SOC_pred_closed'} by
\begin{rocqc}
SOC_pred_closed : ∀ i : [0..n|], SOC_pred (FS i) → SOC_pred (FW i)
\end{rocqc}
That is, we only require $n + 1 \in A \To n \in A$ for $A \in \SOC_n$. The stronger condition \rocqi{SOC_pred_closed'} then follows by induction on the proof of \rocqi{j ⪯ i} (using \rocqi{Sle_rect}).

As mentioned in the introduction to this section, we use a shallow embedding to formalise the internal logic of $\TOT$ in Rocq. This means that we use the semantics of the logic in $\TOT$ to represent types, terms, and proof derivations. That is, types are objects, (well-typed) terms are morphisms from their typing context to their type, and proof derivations are metatheoretic (Rocq) proofs. The definitions of term constructors are then given by their interpretations according to \cref{def:logic-semantics}, whereas the inference rules are verified as lemmas using semantic entailment. In this way, we obtain a small combinator library with which we can construct and reason about terms and formulas as if they were syntactic objects.

For instance, we have the internal conjunction 
\begin{rocqc}
conjI : Ω × Ω ⟶ Ω
\end{rocqc}
introduced in \cref{def:internal-heyting}, where \rocqi{Ω : Object} is the subobject classifier. We can define the respective term former as follows.
\begin{rocqc}
Definition conj {Γ} (P Q : Γ ⟶ Ω) : Γ ⟶ Ω := conjI ∘ ⟨P, Q⟩.
\end{rocqc}
Here, \rocqi{⟨P, Q⟩} denotes categorical pairing. Note that \rocqi{conj} corresponds to the typing rule \ruleref{rule:conj}: given two \emph{semantic} terms \rocqi{P} and \rocqi{Q} of type \rocqi{Ω} in context \rocqi{Γ}, it constructs another proposition in the same context. Also note that its definition follows closely the semantics given in \cref{def:logic-semantics}.

As another example, consider the universal quantifier, corresponding to the typing rule \ruleref{rule:forall}:
\begin{rocqc}
allI : ∀ {X}, (X ⇒ Ω) ⟶ Ω
Definition all {Γ} A (P : Γ × A ⟶ Ω) : Γ ⟶ Ω := allI ∘ λ(P).
\end{rocqc}
where \rocqi{λ(P)} is the exponential transpose of \rocqi{P}. Here, \rocqi{P} is a semantic proposition in context \rocqi{Γ × A}, that is, \rocqi{Γ} extended with a new variable of type \rocqi{A}.

Given the embedding of our term language into Rocq, all we have left to do in order to complete the shallow embedding is to verify semantically the inference rules of the logic. First, recall the semantic provability relation $\sementails{\Gamma}{P}{Q}$ from \cref{def:logic-semantics}.
\begin{rocqc}
Definition entails {Γ} (P Q : Γ ⟶ Ω) : Prop :=
  ∀ n γ, P n γ n → Q n γ n.
\end{rocqc}
The inference rules of the logic (\cref{fig:structural-logical-rules,fig:logic-equalities}) can then be proved as separate lemmas. For example, the following lemma states conjunction introduction (\ruleref{rule:and-intro}).
\begin{rocqc}
Lemma conj_intro {Γ} {R P Q : Γ ⟶ Ω} :
  ...
  R ⊢ conj P Q.
\end{rocqc}
Perhaps a bit confusingly, we use the \rocqi{⊢} symbol for provability in the formalisation. This is because we would like to treat the shallow embedding of the internal language as a combinator calculus, so we prefer to use syntactic notations.

An interesting aspect of our shallow embedding is that there are no variable names: we work with purely categorical combinators. Since terms are represented as morphisms from their context to their type, substitution can simply be implemented by precomposition. For instance, the rule \ruleref{rule:forall-intro} is formalised as follows.
\begin{rocqc}
Lemma all_intro {Γ A} (R : Γ ⟶ Ω) (P : Γ × A ⟶ Ω) :
  R ∘ π₁ ⊢ P →
  R ⊢ all A P.
\end{rocqc}
Here, the first projection \rocqi{π₁} on the left hand side of the premise is a weakening substitution. It is necessary to bring \rocqi{R} into the same context as \rocqi{P}, which has an additional free variable of type \rocqi{A}. Another example is the rule \ruleref{rule:forall-elim} below.
\begin{rocqc}
Lemma all_elim {Γ A} (P : Γ × A ⟶ Ω) (t : Γ ⟶ A) :
  all A P ⊢ P ∘ ⟨mid, t⟩.
\end{rocqc}
Here, \rocqi{mid : ∀ {X}, X ⟶ X} is the identity morphism. In the conclusion, the expression \rocqi{⟨mid, t⟩} is the categorical version of the substitution $\subst{}{x}{t}$, mapping the last variable to \rocqi{t} and leaving all other variables unchanged.

\subsection{Axioms} \label{sec:rocq-axioms}

In our formalisation, we often have to show equality of morphisms, e.g. when proving categorical laws. Since morphisms are essentially families of functions, this involves showing equality of Rocq functions. In most cases, such equalities do not hold by definition. Hence, we make use of the axiom of \emph{function extensionality}, stating that two functions are equal iff they are equal at every input. This allows us for example to perform case distinction or induction on the input to show equality of the outputs.

In \cref{sec:rocq-obj-mor}, we have seen that a morphism is actually a record, containing a family of functions \emph{together with} a proof of their naturality. This means that proving equality of morphisms requires proving equality of both components, thus in particular, of equality proofs. Such higher equalities are notoriously difficult to deal with in constructive type theory. To simplify matters, we assume the \emph{proof-irrelevance} axiom, which states that any two proofs of the same proposition are equal. This way, we can prove the following lemma, expressing that equality of morphisms follows from equality of their underlying families of functions.
\begin{rocqc}
Lemma morph_inj {X Y} {f g : X ⟶ Y} (e : morph f = morph g) : f = g.
\end{rocqc}

The above two considerations also apply to the other record types, namely \rocqi{Exp_obj} and \rocqi{SOC_obj}. The first component of the latter is a function with codomain \rocqi{Prop}. Thus, to prove equality of inhabitants of \rocqi{SOC_obj} (necessary for e.g. naturality proofs of logical connectives), we need to prove equality of Rocq propositions. For this, we use the \emph{propositional extensionality} axiom, stating that two propositions are equal iff they imply each other.

Some might not be satisfied with our Rocq assumptions. All uses of additional axioms in our formalisation arise from the need to prove \emph{propositional equality} of various Rocq objects. A standard way to remedy this situation is to replace types with \emph{setoids}~\cite{barthe:2003:jfp}, also called \emph{Bishop sets}. The main idea is to equip types with an equivalence relation, which we think of as a customised notion of equality. Then, instead of proving propositional equality of elements, we prove that they are related by the relation corresponding to their type. Thus, by giving appropriate equivalence relations for \rocqi{/\textcolor{black}{Morphism}/}, \rocqi{Exp_obj}, and \rocqi{SOC_obj}, we can eliminate our reliance on the axioms mentioned so far.

A downside of this approach is that we would also need to show that functions preserve the respective equivalence relations, resulting in a longer and more complicated formalisation. For instance, the \rocqi{Object} type defined in \cref{sec:rocq-obj-mor} would become the following record.
\begin{rocqc}
Record Object : Type := 
  { obj : nat → Type
...
\end{rocqc}
Here, \rocqi{Equivalence (obj_eq n)} states that \rocqi{obj_eq n} is an equivalence relation. The first three fields together express that we have a family of setoids indexed by the natural numbers. The field \rocqi{restr_resp} witnesses the fact that restriction respects the equivalence relations. We would need to provide proofs of similar statements for most, if not all, of the operations, including the components of morphisms. To keep the formalisation simple, we have decided to stick with the axioms.

We could get rid of the dependence on the proof-irrelevance axiom by using an \rocqi{SProp} version of the equality type in \rocqi{/\textcolor{black}{Morphism}/}, \rocqi{Exp_obj}, and \rocqi{SOC_obj}. Since we do not rely on definitional equalities between these objects, doing so would not improve the ease of formalisation. It would, however, require some boilerplate for proving various lemmas for the new equality type not present in the Rocq standard library. For these reasons, we decided to stick to the proof-irrelevance axiom.

\section{Related work} \label{sec:related}

Sieczkowski et al. created the ModuRes Rocq library~\cite{sieczkowski:2015:itp}, which provides a general framework for solving recursive domain equations, and can be used to build models of complex programming languages and program logics. The mathematical context for their library is given by ordered families of equivalences (OFEs)~\cite{gianantonio:2002:types} and $\mathcal{M}$-categories~\cite{birkedal:2010:tcs}. They showed that the topos of trees is an $\mathcal{M}$-category, meaning that it provides an appropriate setting for solving recursive domain equations. For this purpose, they only needed a small portion of the structure of the topos of trees. Hence, our formalisation of the topos of trees and its logic is more extensive in this regard. However, they also showed the existence of unique fixed points for locally contractive functors, allowing them to model guarded recursive types missing from our formalisation. Furthermore, they avoid axioms by using setoids (cf. \cref{sec:rocq-axioms}).

Bergwerf~\cite{bergwerf:2022:internship} proved a completeness result for propositional logic with the later modality with respect to the simple step-indexed model of \cref{sec:siprop}. In addition to the rules presented in this report, his completeness theorem depends on an extra axiom, called the comparison rule, which carries a classical flavour. In particular, his calculus does not satisfy the disjunction property, a traditional intuitionistic principle. He also formalised his results in Rocq, using a deep embedding for the syntax of the logic.

The use of the topos of trees as denotational semantics for guarded recursion was first suggested by Birkedal et al~\cite{birkedal:2011:lics,birkedal:2012:lmcs}. They considered not only guarded recursive functions, but also guarded recursive types, for which they generalised the results of Birkedal, St{\o}vring, and Thamsborg~\cite{birkedal:2010:tcs}. Subsequently, Birkedal and M{\o}gelberg~\cite{birkedal:2013:lics} showed that guarded recursive types could be constructed internally by taking fixed points of functions on universes.
...
As alluded to in the introduction, step-indexed logics can be used to construct models of programming languages and program logics with self-referential features, such as Iris~\cite{jung:2018:jfp}. It would be worthwhile to attempt to formalise a model of Iris in the internal logic of the topos of trees. It would be especially interesting to carry out the formalisation in a proof assistant which implements guarded recursion natively, such as guarded cubical type theory~\cite{birkedal:2019:jar}.

\paragraph{Acknowledgements.}
I am grateful to Robbert Krebbers for his patience and kindness, for his feedback and helpful suggestions on this report, for the interesting discussions during our meetings, and for sharing his Rocq expertise.
