...
\begin{items}
    ...
    \item Finally, we have formalised the topos of trees in the Rocq theorem prover~\cite{rocq}, including a shallow embedding of its internal logic. We provide details and technical insights in the report.
\end{items}
...
Thus, the rule \textsc{$\later$-$\exists$-comm} holds for total and inhabited $X$. It turns out that we can characterise this property in the internal logic.
...
\end{rocqc}
Here, \rocqi{⟨P, Q⟩} denotes categorical pairing. Note that \rocqi{conj} corresponds to the typing rule \textsc{conj} (\cref{fig:logic-typing-rules}): given two \emph{semantic} terms \rocqi{P} and \rocqi{Q} of type \rocqi{Ω} (i.e. propositions) in context \rocqi{Γ}, it constructs another proposition in the same context. Also note that its definition follows closely the semantics given in \cref{def:logic-semantics}.
